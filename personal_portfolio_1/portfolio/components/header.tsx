...
export default function Header({ isDark, toggleDarkMode }: HeaderProps) {
  ...
  // Listens for the footer's "Documents ↑" click and glows the button once.
  useEffect(() => {
    const handleHighlight = () => {
      setGlow(true)
      const timer = setTimeout(() => setGlow(false), 2700)
      return () => clearTimeout(timer)
    }
    window.addEventListener("documents-highlight", handleHighlight)
    return () => window.removeEventListener("documents-highlight", handleHighlight)
  }, [])
      ...
      <header className="sticky top-0 z-40 bg-background/80 backdrop-blur-md border-b border-border">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            {/* Icon-only logo for visual balance — no name text */}
            <a
              href="#main-content"
            ...
            <nav className="hidden md:flex items-center gap-6" aria-label="Main navigation">
              ...
              <div className={glow ? "documents-glow" : ""}>
                <DocumentsMenu />
              </div>
            </nav>
          ...
          {isMenuOpen && (
            <nav className="md:hidden pb-4 flex flex-col gap-2" aria-label="Mobile navigation">
              ...
              <div className={`px-4 py-2 ${glow ? "documents-glow" : ""}`}>
                <DocumentsMenu />
              </div>
            </nav>
          )}
